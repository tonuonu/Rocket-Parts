...
module R60_TestRing(){
    ...
    difference(){
        ...
        // No counterbore: the gauge must represent the real joint, and the
        // neck has none. See Task 2.
        R60_CameraBoltPattern()
            translate([0,0,-Overlap])
...
module R60_Neck(){
    ...
    difference(){
        ...
        // No counterbore. The neck is modelled flange-first, so z=0 is the
        // face that presses against the camera - a counterbore cut from z=0
        // would open into the mating face. The aft face mates with nothing
        // (tube end lands at r=28.4-30, bolt circle is r=18.98), so the head
        // sits proud there and the full 5mm flange is grip:
        //   M3x10 -> 5.0mm protrusion into a 5.7mm insert. Cannot bottom out.
        R60_CameraBoltPattern()
            translate([0,0,-Overlap])
...
Expected: `0 check(s) failed`, e-bay 160.0 mm and chute bay 130.0 mm, OD 60.0, bore 56.8, and 0.4 mm diametral
...
Add one entry to the existing `GENUS` dict — do not rewrite it:
...
> **GENUS entries are additive.** Each task ADDS its own part's entry to the existing
> `GENUS` dict — it never rewrites the dict wholesale. Task 5 caught this: a pasted
> full-dict snippet silently dropped Task 4's `5:` entry, disabling that part's topology
> check without any test failing. Add your key, leave every existing key alone.
...
GENUS[2] = 3   # was 1; the door and switch hole each add a handle
GENUS[7] = 4
```
...
### Task 7: Spring separation and shear-pin joint

Replaces the abandoned cam-ramped bayonet (spec §4.2). Two independent paths must
separate the airframe at apogee, and **neither may depend on the other's actuator**:

- **Primary** — servo 1 releases a ball-lock that frees a compressed spring; the spring
  drives the sections apart with more than the pins' shear load
- **Backup** — the motor's ejection charge pressurises the chute bay and breaks the same
  pins, with no electronics involved at all

**The placement constraint that makes this work.** The two shear pins must bridge the
**actual separable airframe joint**, through both tube walls at the section split. Pins that
only retain the spring internally leave the ejection charge fighting the ball-lock detent
instead, and the independence property — the entire reason this mechanism was chosen over a
bayonet — is silently lost.

**Files:**
- Modify: `Rocket60.scad`, `R60Lib.scad`
- Modify: `tools/verify_rocket60.py`

**Interfaces:**
- Consumes: `R60_Body_ID`, `R60_Coupler_OD`, `R60_Chute_L`, `IDXtra`, `Overlap`
- Produces: part 8 = spring/ball-lock carrier; shear-pin holes in parts 3 and 5

- [ ] **Step 1: Establish the geometry against the existing libraries**

Read `SpringThingBooster.scad` (ball retainer, parametric on `Body_ID`, proven at 65 mm in
`Rocket65.scad`), `CableReleaseBBMini.scad` (release catch, live in `Rocket6551.scad`), and
`SpringEndsLib.scad` (spring seats). The `CS4323` spring is OD 44.30 / ID 40.50 / free 200 /
coil-bound 22.

Do **not** copy those modules in. Instantiate or adapt them at `Body_ID = 56.8`, and where a
module cannot be used at this diameter, build the equivalent and say so.
...
PIN_BC_R = 28.4      # through the tube wall at the joint
```

```python
    if 8 in m:
        c += [("spring carrier OD", bore(a(8, "stl"), *SHEARPIN_BAND)[1], 56.40, 0.15),
              ("spring carrier bore takes CS4323", bore(a(8, "stl"), *SHEARPIN_BAND)[0], 44.80, 0.30)]
        if 3 in m:
            tube_id, _ = bore(a(3, "stl"), *TUBE_BAND)
            c += [("carrier clears chute bore", tube_id - bore(a(8, "stl"), *SHEARPIN_BAND)[1], 0.4, 0.15)]
```

- [ ] **Step 3: Run and confirm it fails**

`python3 tools/verify_rocket60.py 8` → `RuntimeError: render of part 8 failed`.

- [ ] **Step 4: Implement**

Build the spring carrier (part 8) and add the shear-pin holes to the chute bay tube (part 3)
and the e-bay aft bulkhead (part 5) so a pin passes through both walls at the joint.

- [ ] **Step 5: Verify, export, commit**

Full regression, then export `08_SpringCarrier.stl` and re-export parts 3 and 5.
